...
<body style="margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;background:#f5f5f5">
  <div style="max-width:560px;margin:24px auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb">
    <div style="background:${PA_DARK};padding:16px 24px">
      <span style="color:#fff;font-size:18px;font-weight:bold">PetroAlianza</span>
    </div>
...
export function signatureRequestTemplate(params: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="margin:0 0 12px;font-size:14px;color:#374151">
        Se requiere su firma como <strong>${params.role}</strong> en la <strong>Nota #${params.numero}</strong>.
      </p>
      <p style="margin:0 0 24px;font-size:14px;color:#374151">
        Haga clic en el siguiente botón para revisar y firmar el documento:
      </p>
      <div style="text-align:center;margin:0 0 24px">
        <a href="${params.url}" style="display:inline-block;background:${PA_ORANGE};color:#fff;padding:12px 32px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:14px">
          Revisar y Firmar
        </a>
      </div>
      <p style="margin:0;font-size:12px;color:#9ca3af">
        Este enlace expira en 7 días. Si no puede hacer clic en el botón, copie y pegue esta URL en su navegador:<br>
        <a href="${params.url}" style="color:${PA_ORANGE};word-break:break-all">${params.url}</a>
      </p>
    `),
  };
}
...
export function allSignedTemplate(params: {
  ...
  return {
    ...
    html: layout(`
      ...
      <p style="margin:0 0 12px;font-size:14px;color:#374151">
        Hola <strong>${params.creatorName}</strong>,
      </p>
      <p style="margin:0 0 12px;font-size:14px;color:#374151">
        Todas las firmas han sido completadas en la <strong>Nota #${params.numero}</strong>.
      </p>
